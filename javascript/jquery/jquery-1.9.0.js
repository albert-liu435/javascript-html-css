/*!
http://rapheal.sinaapp.com/tag/%e6%ba%90%e7%a0%81/page/2/
...
(function (window, undefined) {
  ...
  //jQuery暴露一个全局的命名空间jQuery（为了书写更简单，一个简写就是$，幸好Javascript用$来做变量命名是合法的）
  //实际上jQuery是一个函数，为什么要这样设计呢，是因为：

  // 函数也是对象，于是在jQuery这个命名空间上可以绑定工具方法
  // 函数可以有原型prototype，每当通过dom = $(“#id”)取得的所谓jQuery对象，本质就是dom = new jQuery(‘#id’);
  // 如果懂得原型的话，就知道如果在jQuery的原型上绑定方法，像上边那样生成的实例dom可以调用这些方法。
